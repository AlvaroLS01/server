...
public class DocumentoVentaImpresionServicio {
    ...
    public Optional<DocumentoVentaImpresionResultado> imprimir(String uidDocumento, OpcionesImpresionDocumentoVenta opciones) {
        ...
        try (ByteArrayOutputStream salida = new ByteArrayOutputStream()) {
            saleDocumentService.printDocument(salida, datosSesionRequest.getDatosSesionBean(), uidDocumento,
            ...
            DocumentoVentaImpresionResultado resultado = construirResultado(uidDocumento, configuracionImpresion, contenido);
    ...
    private PrintDocumentDTO construirPeticionImpresion(String uidDocumento, OpcionesImpresionDocumentoVenta opciones) {
        ...
        parametros.put("esDuplicado", opciones.esCopia());
        opciones.getParametrosPersonalizados().forEach(parametros::put);

        return printRequest;
    }

    private DocumentoVentaImpresionResultado construirResultado(String uidDocumento, PrintDocumentDTO configuracionImpresion,
            byte[] contenido) {
        boolean copia = Boolean.TRUE.equals(configuracionImpresion.getCopy());
        boolean inline = Boolean.TRUE.equals(configuracionImpresion.getInline());
        String mimeType = configuracionImpresion.getMimeType();
        String nombreArchivo = normalizarNombreArchivo(configuracionImpresion.getOutputDocumentName(), mimeType);

        return new DocumentoVentaImpresionResultado(
                uidDocumento,
                copia,
                inline,
                mimeType,
